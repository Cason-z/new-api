...
func shouldBridgeChatImageRequest(c *gin.Context, relayFormat types.RelayFormat, request dto.Request) bool {
	...
	return common.IsImageGenerationModel(chatRequest.Model) || service.ShouldRouteChatImageIntent(chatRequest)
}
...
func buildImageRequestFromChatRequest(chatRequest *dto.GeneralOpenAIRequest) (*dto.ImageRequest, bool, error) {
	...
	prompt := service.ExtractLatestUserText(chatRequest.Messages)
	...
	if service.ShouldRouteChatImageIntent(chatRequest) {
		model = service.ChatImageIntentTargetModel
		width, height = imageDimensionsFromSize(size)
	}
